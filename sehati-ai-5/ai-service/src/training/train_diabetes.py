...
def train():
    ...
    logger.info(f"Starting {disease_name.title()} Model Training Pipeline")
    
    base_dir = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
    data_path = os.path.join(base_dir, f'data/{disease_name}.csv')
    ...
    if not os.path.exists(data_path):
        ...
        df_mock = pd.DataFrame({
            'HbA1c': np.random.normal(5.5, 1.5, 500),
            'BMI': np.random.normal(28, 5, 500),
            'Age': np.random.randint(20, 80, 500),
            'Family_History': np.random.choice(['Yes', 'No'], 500),
    ...
    # 15. Save Pipeline and Model (Joblib)
